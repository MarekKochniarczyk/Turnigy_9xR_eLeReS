...
struct t_alarmControl AlarmControl = {
		100, 0, 10, 2 };
...
#ifdef VERSION3
		{
				CS_VOFS, CS_VOFS, CS_VOFS, CS_VOFS, CS_VBOOL, CS_VBOOL,
				CS_VBOOL,
				CS_VCOMP, CS_VCOMP, CS_VCOMP, CS_VCOMP, CS_VBOOL, CS_VBOOL,
				CS_TIMER, CS_VOFS };
#else	
...
uint8_t putsTelemValue(uint8_t x, uint8_t y, uint8_t val, uint8_t channel, uint8_t att) {
	...
	if (Unit == 'v') {
		...
		unit = 'v';
		if (!(att & NO_UNIT))
			lcd_putcAtt(lcd_lastPos, y, Unit, att);
	} else {
...
int16_t getValue(uint8_t i) {
	...
	if (i < 7)
		return calibratedStick[i];		//-512..512
	if (i < PPM_BASE)
		return 0;
		...
		return x * 2;
	} else if (i < CHOUT_BASE + NUM_CHNOUT)
		return ex_chans[i - CHOUT_BASE];
...
//				pgm_read_byte(stickScramble+g_eeGeneral.stickMode*4+idx ) ;
		...
		if (thrChan && g_eeGeneral.throttleReversed)
...
void doBackLightVoice(uint8_t evt) {
	...
	if (evt)
		backlightKey(); // on keypress turn the light on 5*100
...
const static prog_uint8_t APM rate[8] = {
		0, 0, 100, 40, 16, 7, 3, 1 };
...
//		MixCounter += 1 ;
	if (tick10ms == 0)
		return; //make sure the rest happen only every 10ms.
...
//					}
			...
			if (pvad->haptic) {
				audioDefevent((pvad->haptic > 1) ? ((pvad->haptic == 3) ? AU_HAPTIC3 : AU_HAPTIC2) :
				AU_HAPTIC1);
			}
...
//    }
	...
	t0 = getTmr16KHz() - t0;
	if (t0 > g_latency.g_timeMain)
		g_latency.g_timeMain = t0;
